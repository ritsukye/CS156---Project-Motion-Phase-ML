...
Chen et al.~\cite{11306882} investigated methods for improving frequency 
...
Pedregosa et al. \cite{scikit-learn} introduced scikit-learn as an open-source machine learning library for Python, providing efficient implementations of classical ML algorithms, including classification, regression, clustering, and model selection tools. The library was designed around a consistent API, making it easy to swap models and build pipelines. It was evaluated across multiple benchmark datasets, demonstrating competitive performance with minimal code. A limitation is that scikit-learn does not natively support GPU acceleration, making it slower than deep learning frameworks for large datasets. In our project, scikit-learn was the primary library used to implement and tune all nine classical models, including Random Forest, SVM, KNN, Decision Tree, Naive Bayes, AdaBoost, Gradient Boosting, and HistGradientBoosting.

\subsection{XGBosst: A Scalable Tree Boosting System}
Chen and Guestrin \cite{chen2016xgboost} introduced XGBoost as a scalable and regularized gradient boosting framework that builds trees sequentially, where each tree corrects errors of the previous one. Key innovations include regularization terms to prevent overfitting, parallel computation for speed, and built-in handling of missing values. The method was evaluated on multiple benchmark datasets and Kaggle competitions, achieving state-of-the-art results on tabular data. A limitation is that XGBoost requires careful hyperparameter tuning and can overfit on small datasets. In our project, XGBoost achieved 52\% accuracy and 0.52 F1-Macro on the motion phase classification task.

\subsection{Random Forests}
Breiman \cite{breiman2001random} introduced Random Forests as an ensemble learning method that constructs multiple decision trees during training and outputs the class that is the mode of the individual trees' predictions. The key idea is that combining many decorrelated trees reduces variance without increasing bias. The method was evaluated on several benchmark datasets achieving competitive accuracy while remaining robust to overfitting. A limitation is that Random Forests can be computationally expensive with large numbers of trees and high-dimensional data. In our project, Random Forest achieved 53\% accuracy on the motion phase classification task, making it one of the stronger classical models.

\subsection{Support-Vector Networks}
Cortes and Vapnik \cite{cortes1995svm} proposed Support Vector Machines (SVM) as a supervised learning method that finds the optimal hyperplane separating classes in high-dimensional feature space using a kernel trick. The RBF kernel was shown to handle non-linearly separable data effectively by projecting features into higher dimensions. The method was evaluated on binary classification tasks, achieving strong generalization even with small training sets. A limitation is sensitivity to kernel and regularization parameter selection. In our project, SVM-RBF achieved 55\% accuracy, the second highest among all models, demonstrating strong discrimination between motion phases in the engineered feature space.

\subsection{Greedy Function Approximation: A Gradient Boosting Machine}
Friedman \cite{friedman2001gbm} introduced Gradient Boosting Machines as a general functional gradient descent framework that builds additive models in a stage-wise fashion, fitting each new tree to the residual errors of the previous ensemble. The method was shown to achieve strong predictive performance on both regression and classification tasks across multiple benchmark datasets. Key innovations include the use of shrinkage (learning rate) and subsampling to improve generalization. A limitation is that gradient boosting is sensitive to hyperparameters and can overfit if the number of trees is too large relative to the dataset size. In our project, both Gradient Boosting and HistGradientBoosting variants were implemented, achieving 38\% and 46\% accuracy, respectively, on the motion phase classification task.

\subsection{A Decision-Theoretic Generalization of Online Learning and Application to Boosting}
Freund and Schapire \cite{freund1997adaboost} proposed AdaBoost as a boosting algorithm that combines multiple weak learners into a strong classifier by iteratively reweighting misclassified samples. Each subsequent weak learner focuses more on previously misclassified examples, progressively improving overall classification performance. The method was evaluated on binary classification tasks, demonstrating that even slightly better than chance weak learners can produce a strong ensemble. A limitation is sensitivity to noisy data and outliers since misclassified samples receive increasingly large weights. In our project, AdaBoost achieved 41\% accuracy on the six-class motion phase classification task, performing moderately below the top ensemble methods.




\subsection{Summary}
Overall, prior work supports several key design decisions in our pipeline. Sliding window segmentation is a standard approach for transforming time-series data into model-ready features. Validation strategy plays a critical role, with LOSO providing a more realistic estimate of cross-subject generalization than K-Fold. Window size selection is highly task-dependent and must be tuned carefully. Finally, feature selection and sensor fusion are both important for improving model performance, particularly when working with wearable biosensor data.
...
This section evaluates the full motion phase classification pipeline. We first report held-out test set results after tuning (4.1), then compare the ANN against classical models (4.2), followed by cross-validation results under both standard and LOSO protocols (4.3--4.4). We then analyse the effect of window size (4.5), per-class metrics (4.6), confusion matrices (4.7), error patterns (4.8), and system integration (4.9).
